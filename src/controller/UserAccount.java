		...
		if (request.getParameter("action").equals("information")) {
			...
			if (!isEmailAddress(email))
				...
				if (actual_user.saveToDb()) {
					...
					FlashMessenger.getMessenger(session).addErrorMessage("The modification of your account failed");
			}
			...
			String c_new_password = (String) request.getParameter("Confirm New Password");
			
			if (password.equals("") || new_password.equals("") || c_new_password.equals(""))
				...
				FlashMessenger.getMessenger(session).addErrorMessage("Wrong password");
			else if (!new_password.equals(c_new_password))
				FlashMessenger.getMessenger(session).addErrorMessage("New Password and Confirm New Password don't match");
			else {
				if (actual_user.saveToDb()) {
					FlashMessenger.getMessenger(session).addSuccessMessage("Your account has been changed");
					response.sendRedirect("/jweb/User/Account");
					return ;
				}
				else
			...
			if (password.equals(""))
				FlashMessenger.getMessenger(session).addErrorMessage("You have to enter your password");
			else if (!actual_user.getPassword().equals(password))
				FlashMessenger.getMessenger(session).addErrorMessage("Wrong password");
			else {
				if (actual_user.eraseToDb()) {
					FlashMessenger.getMessenger(session).addSuccessMessage("Your account has been erase");
					response.sendRedirect("/jweb/Home");
					return ;					
				}
				else
